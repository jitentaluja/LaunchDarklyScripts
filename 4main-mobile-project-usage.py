...
PROJECT_KEY = "afterpay-mobile"
ENVIRONMENT_KEY_1 = "production"
...
def get_flag_usage():
    ...
    filename = "output-usage-mobile-" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    file = open(filename, 'w')

    flagDetails = ("LD Project Name" + ': '+ "Flag name" + ': ' + "Tags" + ": " + "Maintainer email" + ": " + "Launched?" + ": " + "Client side?" + ": " + "Client side - using mobile key?" + ": " + "Client side - using environment id?" + ": " + "Temporary = TRUE Permanent = FALSE" + ": " + "Creation Date" + ": " + "Archive Date" + ": " + "Flag Archived" + ": " + "Prod-Env-1 (evaluations)" + ": " + "Prod-Env-1 (status)" + ": : : : : " + "Total evaluations (millions)")
    file.write(f'{flagDetails} \n')
